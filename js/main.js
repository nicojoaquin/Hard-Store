...
const cartNumbers = () => {
  let productNumbers = parseInt(localStorage.getItem("cart"));  //Almacenamos el valor del storage.

  if(productNumbers){
    localStorage.setItem('cart', productNumbers + 1); //Si ya existe el valor, al hacer click, se suma uno.
    counter.textContent = productNumbers + 1
  } else{
    localStorage.setItem('cart', 1);  //Obtenemos del storage la cantidad 1 en el carrito al crear el storage.
    counter.textContent = 1;
  }
}

const loadCart = () => {  //Al recargar la pagina, sigue el valor del carrito con el mismo valor del storage.
  let productNumbers = parseInt(localStorage.getItem("cart"));
